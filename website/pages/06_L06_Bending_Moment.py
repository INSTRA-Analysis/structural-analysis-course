...
st.caption("Prerequisites: Lessons 1–5  ·  Time: ~75 minutes")
st.markdown("""
...
with col_ctrl:
    L   = st.slider("L (m)", 5.0, 15.0, 10.0, 0.5, key="bmd_L")
    x_A = st.slider("Pin A (m)", 0.0, L * 0.45, 2.0, 0.5, key="bmd_xA")
    x_B = st.slider("Roller B (m)", x_A + 0.5, L, min(8.0, L), 0.5, key="bmd_xB")

    P1  = st.slider("Midspan load (kN)", -80.0, 0.0, -20.0, 5.0, key="bmd_P1")
    w1  = st.slider("Full-span UDL (kN/m)", -15.0, 0.0, -5.0, 1.0, key="bmd_w1")
    P_l = st.slider("Left tip load (kN)", -40.0, 0.0, 0.0, 5.0, key="bmd_Pl")
    P_r = st.slider("Right tip load (kN)", -40.0, 0.0, 0.0, 5.0, key="bmd_Pr")

    show_peaks  = st.checkbox("Annotate peak values", value=True)
    show_vzero  = st.checkbox("Show V=0 lines", value=True)

    geo   = {"L_total": L, "x_A": x_A, "x_B": x_B}
    loads = []
    mid   = (x_A + x_B) / 2
    if P1 != 0: loads.append({"type": "point", "x": mid,  "magnitude": P1})
    if w1 != 0: loads.append({"type": "udl",   "x1": 0.0, "x2": L, "w": w1})
    if P_l != 0: loads.append({"type": "point", "x": 0.0, "magnitude": P_l})
    if P_r != 0: loads.append({"type": "point", "x": L,   "magnitude": P_r})

with col_plot:
    if not loads:
        st.info("Add at least one load using the sliders to see the diagrams.")
    else:
        try:
            ...
            if show_peaks:
                for idx, color, arr, ax_ in [
                    (M.argmax(), "#1B5E20", M, ax_m),
                    (M.argmin(), "#C62828", M, ax_m),
                    (V.argmax(), "#0D47A1", V, ax_s),
                    (V.argmin(), "#B71C1C", V, ax_s),
                ]:
                    if abs(arr[idx]) > 0.5:
